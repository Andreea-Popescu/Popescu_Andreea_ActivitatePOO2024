...
int main() {
    ...
    cout << "Introduceti numarul de RANDURI pentru matricea de telefoane: ";
    cin >> n;

    cout << "Introduceti numarul de COLOANE pentru matricea de telefoane: ";
    cin >> m;

    Telefon** matriceTelefoane = new Telefon * [n];
    for (int i = 0; i < n; i++) {
        matriceTelefoane[i] = new Telefon[m];
    }

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cout << "Cititi datele pentru telefonul de pe pozitia [" << i << "][" << j << "]:" << endl;
            cin >> matriceTelefoane[i][j];
        }
    }

    cout << endl << "Matricea de telefoane:" << endl;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cout << "Telefon [" << i << "][" << j << "]:" << endl;
            cout << matriceTelefoane[i][j] << endl;
        }
    }

    for (int i = 0; i < n; i++) {
        delete[] matriceTelefoane[i];
    }
