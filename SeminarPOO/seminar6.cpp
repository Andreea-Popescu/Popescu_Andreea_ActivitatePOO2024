...
public:
	...
	// OPERATORI DE CAST  | FACEM FORMA EXPLICIT PT MAI MULTI OPERATORI INTR-O CLASA !!!
	...
	// SUPRAINCARCARE OPERATOR < (COMPARATIE) DUPA NRANGAJATI !!!
...
int main() {
	...
	// APELARE OPERATORI CAST !!!
	int nrAngajati = (int)c2;
	float salariiTotale = (float)c2;

	if (c < c2) {
		// c.operator<(c2);
		cout << "C este mai mic decat C2";
	}

	// APELARE OPERATOR DE INDEXARE !!!
	try {
		cout << endl << c2[1] << endl;
		cout << c2[-6];
		cout << c2[1];
